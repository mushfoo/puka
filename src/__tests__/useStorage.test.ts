...
describe.skip('useStorage', () => {
  ...
  it('updates a book successfully', async () => {
    ...
    expect(result.current.books.find((b) => b.id === 1)?.title).toBe(
      'Updated Title'
    )
    expect(mockStorageService.updateBook).toHaveBeenCalledWith(1, {
      title: 'Updated Title',
    })
  })
  ...
  it('updates progress with automatic status changes', async () => {
    ...
    expect(mockStorageService.updateBook).toHaveBeenCalledWith(
      2,
  ...
  it('filters books by status', async () => {
    ...
    const currentlyReadingBooks =
      result.current.getFilteredBooks('currently_reading')
