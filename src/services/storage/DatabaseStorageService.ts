...
import {
  type Book,
  type StreakHistory,
  type EnhancedStreakHistory,
  type EnhancedReadingDayEntry,
} from "@/types";
...
export class DatabaseStorageService implements StorageService {
  ...
  private async authenticatedFetch(
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        console.log(
          `API Request: ${options.method || "GET"} ${endpoint} - ${response.status}`,
        );
      }
  ...
  async withTransaction<T>(
    ...
    try {
      // Begin transaction
      const beginResponse = await this.authenticatedFetch(
      ...
      // Commit transaction
      const commitResponse = await this.authenticatedFetch(
      ...
      this.logResponse("/api/transaction/commit", {
        transactionId,
        success: true,
      });
      return result;
      ...
      try {
        ...
        this.logResponse("/api/transaction/rollback", {
          transactionId,
  ...
  async processBatches<T, R>(
    ...
    const effectiveBatchSize = Math.min(
      batchSize,
      DatabaseStorageService.MAX_BATCH_SIZE,
    );
    const batches: T[][] = [];
      ...
      while (attempts < DatabaseStorageService.MAX_RETRY_ATTEMPTS) {
        try {
          ...
          if (attempts < DatabaseStorageService.MAX_RETRY_ATTEMPTS) {
            ...
            console.warn(
              `Batch ${i + 1} failed, retrying in ${delay}ms (attempt ${attempts})`,
            );
          }
        }
      }
  ...
  private logRequest(method: string, endpoint: string, data?: unknown): void {
    if (process.env.NODE_ENV === "development") {
      console.log(
        `[DatabaseStorageService] ${method} ${endpoint}`,
        data ? { data } : "",
      );
    }
  }
  ...
  private mapDbBookToFrontend(dbBook: DbBook): Book {
    return {
      ...
      dateStarted: dbBook.dateStarted
        ? new Date(dbBook.dateStarted)
        : undefined,
      dateFinished: dbBook.dateFinished
        ? new Date(dbBook.dateFinished)
        : undefined,
  ...
  private mapFrontendBookToDb(frontendBook: Partial<Book>): Partial<DbBook> {
    const dbBook: Partial<DbBook> = {
      ...
      dateStarted: frontendBook.dateStarted
        ? frontendBook.dateStarted.toISOString()
        : null,
      dateFinished: frontendBook.dateFinished
        ? frontendBook.dateFinished.toISOString()
        : null,
    };
   ...
   * @private
   */
  private mapDbStreakHistoryToFrontend(
    dbStreakHistory: DbStreakHistory,
  ): StreakHistory {
    return {
      readingDays: new Set(
        Array.isArray(dbStreakHistory.readingDays)
          ? dbStreakHistory.readingDays
          : Object.values(dbStreakHistory.readingDays || {}),
      ),
   ...
   * @private
   */
  private mapFrontendStreakHistoryToDb(
   ...
   * @private
   */
  private mapDbEnhancedStreakHistoryToFrontend(
    dbEnhancedHistory: DbEnhancedStreakHistory,
  ): EnhancedStreakHistory {
    return {
      readingDays: new Set(
        Array.isArray(dbEnhancedHistory.readingDays)
          ? dbEnhancedHistory.readingDays
          : Object.values(dbEnhancedHistory.readingDays || {}),
      ),
      ...
      readingDayEntries: (dbEnhancedHistory.readingDayEntries || []).map(
        (entry: DbReadingDayEntry) => ({
          date: entry.date,
          source: entry.source,
   ...
   * @private
   */
  private mapFrontendEnhancedStreakHistoryToDb(
    enhancedHistory: EnhancedStreakHistory,
  ): DbEnhancedStreakHistory {
    return {
  ...
  private convertLegacyToEnhanced(
    legacyHistory: StreakHistory,
  ): EnhancedStreakHistory {
    const readingDayEntries: EnhancedReadingDayEntry[] = Array.from(
      legacyHistory.readingDays,
    ).map((date) => ({
      date,
   ...
   * @private
   */
  private validateReadingDayEntry(
    entry: unknown,
  ): entry is EnhancedReadingDayEntry {
    return (
      typeof entry === "object" &&
      entry !== null &&
      "date" in entry &&
      ...
      ["manual", "book", "progress"].includes(
        (entry as Record<string, unknown>).source as string,
      ) &&
      ((entry as Record<string, unknown>).bookIds === undefined ||
        Array.isArray((entry as Record<string, unknown>).bookIds)) &&
  ...
  async exportData(): Promise<ExportData> {
    ...
    try {
      ...
      const [books, settings, streakHistory, enhancedStreakHistory] =
        await Promise.all([
          this.getBooks(),
          this.getSettings(),
  ...
  async importData(
    ...
    this.logRequest("POST", "/api/import", {
    ...
    return this.withTransaction(async (_tx) => {
      ...
      try {
        ...
        if (
          data.enhancedStreakHistory &&
          typeof data.enhancedStreakHistory === "object"
        ) {
          try {
            await this.saveEnhancedStreakHistory(data.enhancedStreakHistory);
            this.logResponse("/api/import (enhanced-streak)", {
              imported: true,
            });
          } catch (error) {
            result.errors.push({
    ...
    try {
      ...
      if (filter.dateRange) {
        filteredBooks = mappedBooks.filter((book) => {
          ...
          return (
            bookDate >= filter.dateRange!.start &&
            bookDate <= filter.dateRange!.end
          );
        });
      }
  ...
  private async importBooks(
    ...
    for (let i = 0; i < books.length; i++) {
      ...
      try {
        ...
        const existingBooks = await this.searchBooks(
  ...
  private validateBookData(
    ...
    if (
      ...
      !["want_to_read", "currently_reading", "finished"].includes(book.status)
    ) {
      errors.push({
        row: rowIndex,
        field: "status",
    ...
    if (
      ...
      (typeof book.rating !== "number" || book.rating < 1 || book.rating > 5)
    ) {
      errors.push({
        row: rowIndex,
        field: "rating",
    ...
    if (
      ...
      (typeof book.totalPages !== "number" || book.totalPages < 1)
    ) {
      errors.push({
        row: rowIndex,
        field: "totalPages",
    ...
    if (
      book.dateStarted &&
      book.dateFinished &&
      this.isValidDate(book.dateStarted) &&
      this.isValidDate(book.dateFinished)
    ) {
  ...
  async getStreakHistory(): Promise<StreakHistory | null> {
    ...
    try {
      ...
      const mappedStreakHistory =
        this.mapDbStreakHistoryToFrontend(dbStreakHistory);
  ...
  async saveStreakHistory(
    ...
    try {
      ...
      const mappedStreakHistory =
        this.mapDbStreakHistoryToFrontend(savedDbStreakHistory);
   ...
   */
  async updateStreakHistory(
    updates: Partial<StreakHistory>,
  ): Promise<StreakHistory> {
    ...
    try {
      ...
      // Map frontend updates to database format
      const dbUpdates = this.mapFrontendStreakHistoryToDb(
        updates as StreakHistory,
      );
      ...
      const mappedStreakHistory = this.mapDbStreakHistoryToFrontend(
        updatedDbStreakHistory,
      );
  ...
  async markReadingDay(): Promise<StreakHistory> {
    ...
    try {
      ...
      this.logResponse("/api/streak/mark-day", {
        marked: true,
        date: todayDate,
      });
      return mappedStreakHistory;
  ...
  async getEnhancedStreakHistory(): Promise<EnhancedStreakHistory | null> {
    ...
    try {
      ...
      if (
        cachedResult &&
        typeof cachedResult === "object" &&
        "readingDayEntries" in cachedResult
      ) {
      ...
      if (response.status === 404) {
        ...
        if (legacyHistory) {
          ...
          this.logResponse("/api/streak/enhanced", {
            found: true,
            migrated: true,
          });
      ...
      const mappedEnhancedHistory =
        this.mapDbEnhancedStreakHistoryToFrontend(dbEnhancedHistory);
   ...
   */
  async saveEnhancedStreakHistory(
    enhancedHistory: EnhancedStreakHistory,
  ): Promise<EnhancedStreakHistory> {
    ...
    try {
      ...
      // Map frontend data to database format
      const dbEnhancedHistory =
        this.mapFrontendEnhancedStreakHistoryToDb(enhancedHistory);
      ...
      const mappedEnhancedHistory = this.mapDbEnhancedStreakHistoryToFrontend(
        savedDbEnhancedHistory,
      );
   ...
   */
  async updateEnhancedStreakHistory(
    ...
    try {
      ...
      // Map frontend updates to database format
      const dbUpdates = this.mapFrontendEnhancedStreakHistoryToDb(
        updates as EnhancedStreakHistory,
      );
  ...
  async addReadingDayEntry(
    ...
    try {
      ...
      this.logResponse("/api/streak/entries", {
        added: true,
        date: entry.date,
      });
      return mappedEnhancedHistory;
  ...
  async updateReadingDayEntry(
    ...
    try {
      ...
      if (
        updates.source &&
        !["manual", "book", "progress"].includes(updates.source)
      ) {
        throw new StorageError(
          "Invalid source. Must be manual, book, or progress",
          StorageErrorCode.INVALID_DATA,
   ...
   */
  async getReadingDayEntriesInRange(
    startDate: string,
    endDate: string,
  ): Promise<EnhancedReadingDayEntry[]> {
    this.logRequest(
      "GET",
      `/api/streak/entries?start=${startDate}&end=${endDate}`,
    );
    ...
    try {
      ...
      // Check cache first
      const cacheKey = this.getCacheKey("getReadingDayEntriesInRange", {
        startDate,
        endDate,
      });
      const cachedResult = this.getCachedResult(cacheKey);
      ...
      const response = await this.authenticatedFetch(
        `/api/streak/entries?${params.toString()}`,
      );
  ...
  async migrateToEnhancedStreakHistory(): Promise<EnhancedStreakHistory | null> {
    ...
    try {
      ...
      // Perform migration
      const migrateResponse = await this.authenticatedFetch(
        "/api/streak/migrate",
        {
          method: "POST",
          body: JSON.stringify({
    ...
    try {
      ...
      const csvRows = books.map((book) => {
        const row = [
          ...
          book.dateFinished
            ? book.dateFinished.toISOString().split("T")[0]
            : "",
  ...
  async cancelImportExportOperation(operationId: string): Promise<boolean> {
    ...
    try {
      ...
      if (response.status === 404) {
        this.logResponse(`/api/operations/${operationId}`, {
          cancelled: false,
          reason: "not_found",
        });
        return false;
   ...
   */
  async bulkUpdateReadingDayEntries(
    operations: BulkReadingDayOperation[],
  ): Promise<EnhancedStreakHistory> {
    this.logRequest("POST", "/api/streak/entries/bulk", {
    ...
    try {
      ...
      for (let i = 0; i < operations.length; i++) {
        ...
        if (
          ...
          !["manual", "book", "progress"].includes(operation.entry.source)
        ) {
          throw new StorageError(
        ...
        if (
          operation.updates?.source &&
          !["manual", "book", "progress"].includes(operation.updates.source)
        ) {
          throw new StorageError(
