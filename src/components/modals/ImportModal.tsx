  ...
  const handleFileSelect = useCallback(async (file: File) => {
    ...
    if (!file.name.toLowerCase().endsWith(".csv")) {
      logImportPhase(
        "file-validation",
        null,
        "File validation failed: not a CSV file",
      );
    ...
    try {
      ...
      logImportPhase(
        "csv-data-loaded",
        { rowCount: fullCsvData.length },
        "Full CSV data loaded",
      );
      ...
      if (preview.sampleBooks && preview.sampleBooks.length > 0) {
        try {
          ...
          logImportPhase(
            "streak-analyzed",
            streakPreview,
            "Streak analysis completed",
          );
        } catch (error) {
          logImportPhase(
            "streak-analysis-error",
            error,
            "Failed to analyze streak data",
          );
      ...
      setState((prev) => ({
        ...
        csvData: fullCsvData,
        selectedFormat:
          preview.suggestedFormat ||
          ImportService.getSupportedFormats().find((f) => f.id === "generic")!,
        step:
          preview.success && preview.sampleBooks.length > 0
            ? "preview"
            : "mapping",
        isProcessing: false,
      }));
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-text-secondary text-sm">
          Upload a CSV file from Goodreads, another reading app, or your own
          data export
        </p>
      </div>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className="text-text-secondary text-sm mt-1">
                Supports Goodreads exports and other CSV formats
              </p>
              <p className="text-blue-600 text-xs mt-1">
                💡 Include "Date Started" and "Date Finished" columns to enhance
                your reading streaks!
              </p>
            </div>
          </div>
        )}
      ...
      <div className="mt-6 bg-gray-50 rounded-lg p-4">
        ...
        <div className="bg-white rounded border border-gray-200 p-3 font-mono text-xs overflow-x-auto">
          <div className="text-gray-600">
            Title,Author,Status,Progress,Rating,Notes,Date Started,Date Finished
          </div>
          <div className="text-gray-800">
            "The Great Gatsby","F. Scott
            Fitzgerald","finished",100,5,"Classic","2024-01-01","2024-01-15"
          </div>
          <div className="text-gray-800">
            "1984","George
            Orwell","currently_reading",65,,"Dystopian","2024-01-16",
          </div>
          <div className="text-gray-800">
            "To Kill a Mockingbird","Harper Lee","want_to_read",0,,"On my
            list",,
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3 text-xs text-text-secondary">
          <div>
            <p>
              <strong>Required:</strong> Title, Author
            </p>
            <p>
              <strong>Status:</strong> "want_to_read", "currently_reading",
              "finished"
            </p>
            <p>
              <strong>Progress:</strong> 0-100 | <strong>Rating:</strong> 1-5
            </p>
          </div>
          <div className="text-blue-600">
            <p>
              <strong>📈 For Reading Streaks:</strong>
            </p>
            <p>
              <strong>Date Started:</strong> YYYY-MM-DD format
            </p>
            <p>
              <strong>Date Finished:</strong> YYYY-MM-DD format
            </p>
            <p className="mt-1">
              Dates will add reading days to your streak history!
            </p>
          </div>
        </div>
        <button
      ...
      <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              {state.preview.streakPreview.dateRange.earliest && (
                <div className="col-span-2">
                  ...
                  <span className="ml-2 font-medium text-blue-900">
                    {state.preview.streakPreview.dateRange.earliest.toLocaleDateString(
                      "en-US",
                      {
                        year: "numeric",
                        month: "long",
                    ...
                    -{" "}
                    {state.preview.streakPreview.dateRange.latest?.toLocaleDateString(
                      "en-US",
                      {
                        year: "numeric",
                        month: "long",
              ...
              {state.preview.streakPreview.stats.overlappingPeriods > 0 && (
                <div className="col-span-2">
                  ...
                  <span className="ml-2 font-medium text-blue-900">
                    {state.preview.streakPreview.stats.overlappingPeriods}{" "}
            ...
            {state.preview.streakPreview.warnings.length > 0 && (
              <div className="mt-3 pt-3 border-t border-blue-200">
                ...
                <div className="space-y-1 text-sm text-blue-700">
                  {state.preview.streakPreview.warnings
                    .slice(0, 3)
                    .map((warning, index) => (
                  ...
                  {state.preview.streakPreview.warnings.length > 3 && (
                    <p>
                      ... and {state.preview.streakPreview.warnings.length - 3}{" "}
                      more warnings
                    </p>
                  )}
                </div>
              ...
              {state.preview.sampleBooks.map((book, index) => (
                <div key={index} className="bg-white border rounded-lg p-3">
                  ...
                  <div className="text-sm text-text-secondary">
                    by {book.author} • {book.status?.replace("_", " ")} •{" "}
                    {book.progress}%
        ...
        <div className="flex gap-3">
          ...
          >
            ...
            {state.preview.streakPreview &&
              state.preview.streakPreview.totalDaysToAdd > 0 && (
      ...
      <div className="space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p className="text-sm text-blue-700">
            Map your CSV columns to book fields. Only Title is required - other
            fields are optional.
          </p>
        </div>
            ...
            >
              ...
              <div className="flex-1">
                <select
                  ...
                  onChange={(e) =>
                    handleColumnMappingChange(column, e.target.value)
                  }
    ...
    <div className="space-y-6 text-center">
      ...
      <div>
        ...
        <p className="text-text-secondary">
          Please wait while we import your reading data.
        </p>
      </div>
    </div>
  );
      ...
      <div className="space-y-6 text-center">
        ...
        <div>
          ...
          <div className="space-y-2 text-sm text-text-secondary">
            ...
            {state.importResult.streakResult &&
              state.importResult.streakResult.daysAdded > 0 && (
                <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <p className="text-blue-800 font-medium">
                    📈 Reading Streak Enhanced!
                  </p>
                  <div className="text-xs text-blue-700 mt-1 space-y-1">
                    <p>
                      📅 {state.importResult.streakResult.daysAdded} reading
                      days added to your history
                    </p>
                    {state.importResult.streakResult.newCurrentStreak !==
                      state.importResult.streakResult.oldCurrentStreak && (
                      <p>
                        🔥 Current streak:{" "}
                        {state.importResult.streakResult.oldCurrentStreak} →{" "}
                        {state.importResult.streakResult.newCurrentStreak} days
                      </p>
                    )}
                    {state.importResult.streakResult.newLongestStreak !==
                      state.importResult.streakResult.oldLongestStreak && (
                      <p>
                        🏆 Longest streak:{" "}
                        {state.importResult.streakResult.oldLongestStreak} →{" "}
                        {state.importResult.streakResult.newLongestStreak} days
                      </p>
                    )}
                  </div>
                </div>
