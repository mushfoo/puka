...
export async function handleTransactionRequest(req: ApiRequest, res: ApiResponse, userId: string | null, action?: string) {
  try {
    if (!userId) {
...
async function handleBeginTransaction(req: ApiRequest, res: ApiResponse, userId: string) {
...
async function handleCommitTransaction(req: ApiRequest, res: ApiResponse, userId: string) {
  const { transactionId } = req.body || {};
...
async function handleRollbackTransaction(req: ApiRequest, res: ApiResponse, userId: string) {
  const { transactionId } = req.body || {};
