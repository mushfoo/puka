...
function AppContent() {
    ...
    try {
      ...
      if (book) {
        const newProgress = Math.min(
          100,
          Math.max(0, book.progress + increment)
        )
  ...
  const handleUpdateBook = async (bookId: number, updates: Partial<Book>) => {
    try {
      ...
      if (updatedBook) {
        success(
          `"${updates.title || updatedBook.title}" updated successfully!`,
          {
            title: 'Book Updated',
            duration: 3000,
          }
  ...
  const handleImportComplete = async (result: ImportResult) => {
    try {
        ...
        try {
          ...
          if (booksWithReadingPeriods.length > 0) {
            ...
            if (existingStreakHistory) {
              ...
              const { extractReadingPeriods, generateReadingDays } =
                await import('./utils/readingPeriodExtractor')
          ...
          console.warn(
            'Streak processing failed, but import succeeded:',
            streakError
          )
      ...
      if (
        result.streakResult &&
        (result.streakResult.newCurrentStreak !==
          result.streakResult.oldCurrentStreak ||
          result.streakResult.newLongestStreak !==
            result.streakResult.oldLongestStreak)
      ) {
        setTimeout(() => {
          info(
            '📈 Your reading streaks have been updated! Check your dashboard for the new counts.',
            {
      ...
      showError(
        'Import completed but failed to refresh data. Please reload the page.'
      )
    }
  }
